    ...
    <div>
      ...
      <div class="bg-gray-800 text-white p-4 flex justify-center gap-4">
        ...
        <button
          ...
          [class.bg-purple-600]="userRole === 'employee'"
          [class.bg-gray-600]="userRole !== 'employee'"
          class="px-4 py-2 rounded-lg hover:bg-purple-700 transition"
        >
          Employee (Piyush Kumar)
        </button>
      </div>
...
export class DashboardComponent implements OnInit {
  userRole: 'super_admin' | 'client_admin' | 'employee' = 'super_admin';
  ...
  switchRole(role: 'super_admin' | 'client_admin' | 'employee') {
    this.userRole = role;
    
    // Mock user data based on role
    const mockUser: User = {
      id: role === 'super_admin' ? 'admin_1' : role === 'client_admin' ? 'client_admin_1' : 'emp_1',
      name: role === 'super_admin' ? 'Arthnext Admin' : role === 'client_admin' ? 'Nextloop Admin' : 'John Doe',
      email: role === 'super_admin' ? 'admin@arthnext.com' : role === 'client_admin' ? 'admin@nextloop.com' : 'john@nextloop.com',
      role: role,
      clientId: role !== 'super_admin' ? 'client_1' : undefined
    };
